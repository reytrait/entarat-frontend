...
## [2025-12-31] Monorepo Migration - NestJS Backend

### Monorepo Structure
- **Created monorepo structure** with pnpm workspaces
- **Moved Next.js app to `apps/web/`** - All frontend code now in dedicated directory
- **apps/server**: NestJS backend server
- **packages/shared**: Shared TypeScript types and utilities
- **Updated workspace configuration** to support multiple packages
- **Updated root package.json** with monorepo scripts

### Environment Configuration
- **Created `.env.example` files** for both frontend and backend
- **Frontend environment variables**:
  - `NEXT_PUBLIC_API_URL` - Backend API URL (not exposed to client via Next.js API routes)
  - `NEXT_PUBLIC_WS_URL` - WebSocket server URL (configurable for separate deployment)
- **Backend environment variables**:
  - `PORT` - Server port (default: 3001)
  - `FRONTEND_URL` - Frontend URL for CORS configuration
  - `NODE_ENV` - Environment mode
- **Updated WebSocket connections** to use environment variables with fallbacks

### NestJS Backend Setup
- **Initialized NestJS server** with WebSocket support
- **Created database service** for in-memory game/player/question storage
- **Set up shared types package** for type safety between frontend and backend
- **Configured CORS** for Next.js frontend communication with environment variable support
...
- **Created comprehensive `DEPLOYMENT.md`** with:
  ...
  - Frontend deployment options (Vercel, EC2, Docker)
  - Environment configuration for separate deployments
...
### File Organization
- **Moved all Next.js files** from root to `apps/web/`:
  - `src/` → `apps/web/src/`
  - `public/` → `apps/web/public/`
  - Configuration files (next.config.ts, tsconfig.json, etc.)
- **Created proper package.json** for `apps/web` with workspace dependencies
- **Updated `.gitignore`** for monorepo structure

### Next Steps
- Complete migration of WebSocket gateway from Express to NestJS
- Migrate round-timer service to NestJS
- Create Next.js API routes to proxy HTTP requests to NestJS
- Implement WebSocket proxy through Next.js (or use environment variable approach)

### Benefits
- **Security**: Backend URL not exposed to client (via environment variables and Next.js API routes)
- **Type Safety**: Shared types between frontend and backend
- **Scalability**: Independent deployment of frontend and backend
- **Maintainability**: Clear separation of concerns
- **Flexibility**: Easy to deploy backend to AWS EC2 and frontend to Vercel separately
